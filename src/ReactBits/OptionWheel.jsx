...
const OptionWheel = ({
  ...
  const playTick = useCallback(() => {
    const { soundUrl: tickUrl, soundVolume: tickVolume } = cfgRef.current;
    if (!tickUrl) return;
    const now = performance.now();
    if (now - lastTickRef.current < 70) return;
    lastTickRef.current = now;
    if (!audioRef.current || audioUrlRef.current !== tickUrl) {
      audioRef.current = new Audio(tickUrl);
      audioRef.current.preload = "auto";
      audioUrlRef.current = tickUrl;
    }
    const audio = audioRef.current;
    audio.volume = Math.min(Math.max(tickVolume, 0), 1);
    audio.currentTime = 0;
    audio.play()?.catch(() => {});
  }, []);
  ...
  return (
    <div
      ...
      role="listbox"
      tabIndex={0}
      aria-label="Portfolio project selector"
