...
// GooeyNav has been removed as per the request for hamburger-only navigation.
...
const Landing = () => {
  ...
  const [mainMenuOpen, setMainMenuOpen] = useState(false); // Renamed for clarity
  ...
  const certifications = [
    {
      ...
      image: "/MachineLearningPreview.webp", // Assuming in /public folder
      description:
    ...
    },
    {
      ...
      image: "/AiPreview.webp", // Assuming in /public folder
      description:
    ...
    },
    {
      ...
      image: "/PreviewUX.webp", // Assuming in /public folder
      description:
  ...
  const sendEmail = (e) => {
    ...
    emailjs
      .sendForm(
        "service_sh057qd", // Ensure this is correct
        "template_2vmky28", // Ensure this is correct
        form,
        "CcFtpOjqJJUhsgyIK"  // Ensure this is correct (Public Key)
      )
      .then(
        (result) => {
          console.log("Email sent successfully:", result.text, result);
          alert("Your message has been sent!");
          setContactModalOpen(false);
          form.reset();
        },
        (error) => {
          console.error("Full error object from EmailJS:", error);
          let errorMessage = "Oops! Something went wrong, please try again.";
          if (error) {
            if (typeof error.text === 'string' && error.text.trim() !== '') {
              errorMessage = `Error: ${error.text}`;
            } else if (typeof error.status === 'number') {
              errorMessage = `Error: Status ${error.status}. Check console for details.`;
            } else if (error.message) {
                errorMessage = `Error: ${error.message}`;
            }
          }
          console.error("Error in sending email (processed):", errorMessage);
          alert(errorMessage);
        }
      );
    // Removed e.target.reset() from here; it's now in the .then() block on success.
  };
  ...
  useEffect(() => {
    ...
    if (hero) {
      ...
      const heroTextElements = hero.querySelectorAll(".hero-text-anim > div > span > span"); // Target individual words
  ...
  const handleNavClick = (e, href) => {
    ...
    if (targetElement) {
      // Add a slight offset to account for sticky header height if needed
    ...
    <div className="min-h-screen flex flex-col bg-black text-white relative selection:bg-indigo-500 selection:text-white antialiased">
      ...
      <header className="sticky top-0 z-[1000] bg-black/75 backdrop-blur-lg shadow-2xl shadow-black/20">
        <div className="max-w-7xl mx-auto flex justify-between items-center py-3.5 px-4 sm:px-6 lg:px-8">
          <motion.a // Changed to <a> for semantic correctness if it links to home
            href="#home"
        ...
        <AnimatePresence>
          {mainMenuOpen && (
            <motion.div
              ...
              // For an overlay effect on desktop for the menu:
              ...
              // Or for a full-width dropdown: className="bg-black/90 border-t border-neutral-800/70" 
            >
              <nav className="px-4 py-4 space-y-2"> {/* Increased py */}
      ...
      <main className="flex-grow isolate"> {/* Added isolate for stacking context */}
        ...
        >
          <div className="absolute inset-0 bg-black bg-opacity-70 backdrop-blur-sm" /> {/* Slightly increased opacity */}
            ...
            <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.3, ease: "circOut" }} >
              <div className="hero-text-anim"> {/* This class is used by GSAP for word animation */}
                <SplitText text="Embark on a New Journey"
                  className="text-4xl sm:text-5xl lg:text-6xl font-extrabold text-white text-center !leading-tight" // Added !leading-tight
        ...
        {/* Sections (About, Tech, Projects, etc.) remain structurally similar to your last good version */}
        {/* Small tweaks for consistency and polish might be applied within them */}
        ...
        <section id="about" className="py-20 md:py-24 bg-black relative overflow-hidden">
          <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} transition={{ duration: 0.6, delay: 0.3, type: "spring", stiffness: 150 }} viewport={{ once: true, amount: 0.5 }} >
              <a href="https://drive.google.com/file/d/1dJKCrpJTReFvzrQ3HMdBa_WO_-s52c_t/view?usp=drive_link" target="_blank" rel="noopener noreferrer"
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-5 md:gap-6 place-items-center">
              {techStack.map((tech, index) => (
                ...
                >
                  <div className="absolute inset-0 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-[radial-gradient(300px_circle_at_var(--x)_var(--y),rgba(34,211,238,0.08),transparent)]" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 place-items-stretch">
              ...
              <motion.div initial={{ opacity: 0, y: 20, scale:0.95 }} whileInView={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.5, ease: "easeOut", delay: 0.1 }} viewport={{ once: true, amount: 0.2 }} className="h-full flex" >
                <SpotlightCard className="p-6 flex flex-col justify-between h-full bg-neutral-900/80 hover:bg-neutral-800/70 border border-neutral-800/80 hover:border-indigo-500/60 rounded-xl shadow-xl hover:shadow-indigo-500/20 transition-all duration-300 ease-in-out group cursor-pointer-interactive w-full">
                  {/* ... content from your previous Project 1 card ... */}
                  <div>
                    ...
                    <p className="text-gray-300/90 leading-relaxed mb-6 text-sm">AI-powered web application that identifies plant diseases from images using CNN models. Provides detailed analysis and preventive measures for healthy crop maintenance.</p>
                  </div>
              ...
              {/* Project 2 (structure similar to Project 1) */}
              <motion.div initial={{ opacity: 0, y: 20, scale:0.95 }} whileInView={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.5, delay: 0.2, ease: "easeOut" }} viewport={{ once: true, amount: 0.2 }} className="h-full flex" >
                 <SpotlightCard className="p-6 flex flex-col justify-between h-full bg-neutral-900/80 hover:bg-neutral-800/70 border border-neutral-800/80 hover:border-teal-500/60 rounded-xl shadow-xl hover:shadow-teal-500/20 transition-all duration-300 ease-in-out group cursor-pointer-interactive w-full">
                    {/* ... content from your previous Project 2 card ... */}
                    <div>
                        <div className="flex items-center gap-3 mb-4">
                        ...
                        <div className="mb-5 rounded-lg overflow-hidden border border-neutral-700/80 group-hover:border-teal-500/50 transition-all duration-300 shadow-md aspect-video bg-neutral-800 flex items-center justify-center text-neutral-600">
                          <svg xmlns="http://www.w3.org/2000/svg" className="h-12 w-12" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1"> <path strokeLinecap="round" strokeLinejoin="round" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /> </svg>
                        </div>
                        <p className="text-gray-300/90 leading-relaxed mb-6 text-sm">End-to-end MLOps solution deploying machine learning models via Hugging Face and Flask APIs. Demonstrates real-time predictions with seamless integration.</p>
                      </div>
                      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 border-t border-neutral-700/60 pt-4 mt-auto">
              ...
              {/* Project 3 (structure similar to Project 1) */}
              <motion.div initial={{ opacity: 0, y: 20, scale:0.95 }} whileInView={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.5, delay: 0.3, ease: "easeOut" }} viewport={{ once: true, amount: 0.2 }} className="h-full flex" >
                 <SpotlightCard className="p-6 flex flex-col justify-between h-full bg-neutral-900/80 hover:bg-neutral-800/70 border border-neutral-800/80 hover:border-purple-500/60 rounded-xl shadow-xl hover:shadow-purple-500/20 transition-all duration-300 ease-in-out group cursor-pointer-interactive w-full">
                    {/* ... content from your previous Project 3 card ... */}
                    <div>
                        <div className="flex items-center gap-3 mb-4">
                        ...
                        <div className="mb-5 rounded-lg overflow-hidden border border-neutral-700/80 group-hover:border-purple-500/50 transition-all duration-300 shadow-md aspect-video bg-neutral-800 flex items-center justify-center text-neutral-600">
                          <svg xmlns="http://www.w3.org/2000/svg" className="h-12 w-12" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1"> <path strokeLinecap="round" strokeLinejoin="round" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /> </svg>
                        </div>
                      ...
                      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 border-t border-neutral-700/60 pt-4 mt-auto">
                        ...
                        <a href="https://employee-mangement-system-w79y.onrender.com" target="_blank" rel="noopener noreferrer" className="flex items-center w-full sm:w-auto justify-center px-4 py-2 bg-purple-600 hover:bg-purple-500 text-white text-sm rounded-md font-medium transition-all duration-200 transform hover:scale-105 hover:shadow-lg hover:shadow-purple-500/40" >
                          <span className="mr-2">Live Demo</span>
                          <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}> <path strokeLinecap="round" strokeLinejoin="round" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" /> </svg>
                        </a>
                      </div>
              ...
              {certifications.map((cert, idx) => (
                ...
                >
                  <a href={cert.link} target="_blank" rel="noopener noreferrer" className="flex flex-col w-full bg-gradient-to-br from-neutral-900/90 to-black/95 rounded-xl overflow-hidden shadow-xl hover:shadow-2xl hover:shadow-indigo-500/25 transition-all duration-400 h-full border border-neutral-800/70 hover:border-indigo-500/60 cursor-pointer-interactive" >
                    <div className="relative overflow-hidden aspect-video"> {/* Changed to aspect-video for more standard preview */}
